...
st.markdown("""
    <style>
        ...
        .laeq-box { background-color: #1e1e1e; border-left: 5px solid #ff6952; padding: 12px; border-radius: 4px; margin-bottom: 10px; }
        .laeq-title { font-size: 0.9rem; color: #808080; margin: 0; }
        .laeq-value { font-size: 1.6rem; font-weight: bold; color: #ffffff; margin: 0; }
    </style>
...
translations = {
    "Français": {
        ...
        "tab_global": " Vue globale", "tab_1h": " Données (1h)", "tab_15m": " Données (15min)", "tab_aq": " Données AQ", "tab_alerts": " Alertes",
        ...
        "welcome_global": "###  Bienvenue sur votre Vue Globale", "desc_global": "Sélectionnez les onglets suivants pour analyser le détail des indicateurs acoustiques, de qualité de l'air ou le résumé des alertes.",
        "map_title": "#### Localisation & Indicateurs Cadence (Limonest)", "side_values": "#### Valeurs LAeq"
    },
    "Español": {
        ...
        "tab_global": " Vista global", "tab_1h": " Datos (1h)", "tab_15m": " Datos (15min)", "tab_aq": " Datos AQ", "tab_alerts": " Alertas",
        ...
        "welcome_global": "###  Bienvenido a su Vista Global", "desc_global": "Seleccione las siguientes pestañas para analizar el detalle de los indicadores acústicos, la calidad del aire o el resumen de alertas.",
        "map_title": "#### Ubicación e Indicadores (Limonest)", "side_values": "#### Valores LAeq"
    },
    "Català": {
        ...
        "tab_global": " Vista global", "tab_1h": " Dades (1h)", "tab_15m": " Dades (15min)", "tab_aq": " Dades AQ", "tab_alerts": " Alertes",
        ...
        "welcome_global": "###  Benvingut a la seva Vista Global", "desc_global": "Seleccioneu les pestanyes següents per analitzar el detall dels indicadors acústics, la qualitat de l'aire o el resum d'alertes.",
        "map_title": "#### Ubicació i Indicadors (Limonest)", "side_values": "#### Valors LAeq"
    },
    "Deutsch": {
        ...
        "tab_global": " Gesamtübersicht", "tab_1h": " Daten (1h)", "tab_15m": " Daten (15min)", "tab_aq": " AQ-Daten", "tab_alerts": " Alarme",
        "no_data": "Keine Daten für diese Filter gefunden.", "data_table": "Datentabelle",
        "rows": "Zeilen", "export": " Als CSV exportieren", "missing_key": " API-Schlüssel fehlt",
        "invalid_points": " Ungültiges Format für Messpunkt-IDs", "analyzing": " Analysiere {} Messpunkte...",
        "fetching": "Daten werden geladen...", "no_alerts": "Keine Alarme für diesen Zeitraum gefunden (Dashboard-ID prüfen).",
        "unknown": "Unbekannt", "status_summary": "###  Statusübersicht", "total_alerts": "Alarme insgesamt",
        "val_alerts": " Validiert", "unval_alerts": " Nicht validiert", "open_alerts": " Offen (in Bearbeitung)",
        "chart_title_1": "#### Anzahl der Alarme nach Messpunkt und Typ", "chart_title_2": "#### Identifizierte Quellen (KI)",
        "no_ident": "Keine identifizierten Alarme.", "no_source_info": "Keine Quelleninformationen.",
        "raw_data": "###  Rohdaten", "api_empty": "Die API hat keine Daten zurückgegeben. Prüfen Sie die Daten oder IDs.",
        "welcome_global": "###  Willkommen auf Ihrer Gesamtübersicht", "desc_global": "Wählen Sie die folgenden Registerkarten aus, um Details zu akustischen Indikatoren, Luftqualität oder die Alarmübersicht zu analysieren.",
        "map_title": "#### Standort & Indikatoren (Limonest)", "side_values": "#### LAeq-Werte"
    }
}
...
with st.sidebar:
    ...
    lang = st.selectbox("Idioma / Langue / Llengua / Sprache", ["Français", "Español", "Català", "Deutsch"])
    t = translations[lang]
    st.divider()
...
if btn_run:
    ...
    with st.spinner(t["fetching"]):
        ...
        st.session_state['df_aq'] = None 
        st.session_state['df_alerts'] = get_cadence_alerts(api_key, dashboard_id, d_start, d_end)


# --- RENDERING FUNCTIONS ---
def render_dashboard(df, title_suffix, limit_val, is_aq=False):
    ...
    with col_graph:
        ...
        # AJOUT DE LA LIGNE LIMITE UNIQUEMENT POUR LE BRUIT (PAS EN AQ) ET SI > 0
        if not is_aq and limit_val > 0:
            fig.add_hline(y=limit_val, line_dash="dash", line_color="#ff6952", annotation_text=f"{limit_val} dB", annotation_position="top left")

        y_axis_label = "Concentration (µg/m³)" if is_aq else "Level (dB)"
        fig.update_layout(
            title=f"{title_suffix}", xaxis_title="Time", yaxis_title=y_axis_label,
...
if st.session_state['has_run']:
    if st.session_state['df_1h'] is None and st.session_state['df_15m'] is None and st.session_state['df_aq'] is None and st.session_state['df_alerts'] is None:
        st.error(t["api_empty"])
    else:
        t0, t1, t2, t3, t4 = st.tabs([t["tab_global"], t["tab_1h"], t["tab_15m"], t["tab_aq"], t["tab_alerts"]])
        with t0:
            st.markdown(t["welcome_global"])
            st.write(t["desc_global"])
            st.divider()
            ...
            with col_left:
                # Colonne gauche : 4 lignes de blocs LAeq stylisés en CSS
                st.markdown(f"""
                <div class="laeq-box">
                    <p class="laeq-title">LAeq Journée (06h - 22h)</p>
                    <p class="laeq-value">62.4 dB</p>
                </div>
                <div class="laeq-box">
                    <p class="laeq-title">LAeq Nuit (22h - 06h)</p>
                    <p class="laeq-value">44.1 dB</p>
                </div>
                <div class="laeq-box">
                    <p class="laeq-title">LAeq Maximum détecté</p>
                    <p class="laeq-value" style="color: #ff6952;">88.7 dB</p>
                </div>
                <div class="laeq-box">
                    <p class="laeq-title">LAeq Moyenne 24h</p>
                    <p class="laeq-value">58.2 dB</p>
                </div>
                """, unsafe_allow_html=True)
            ...
            with col_right:
                ...
                map_html = """
                <iframe 
                    src="https://maps.google.com/maps?q=ACOEM%20Limonest&t=&z=14&ie=UTF8&iwloc=&output=embed" 
                    width="100%" 
                    height="380" 
        ...
        with t1: render_dashboard(st.session_state['df_1h'], t["hourly"], limit_db_val, is_aq=False)
        with t2: render_dashboard(st.session_state['df_15m'], t["short"], limit_db_val, is_aq=False)
        with t3: render_dashboard(st.session_state['df_aq'], t["tab_aq"], limit_db_val, is_aq=True)
        with t4: render_alerts(st.session_state['df_alerts'])
else:
    ...
    elif lang == 'Català': msg = " Obre les seccions a la barra lateral per configurar i carregar dades."
    else: msg = " Öffnen Sie die Bereiche in der Seitenleiste, um Daten zu konfigurieren und zu laden."
    st.info(msg)
